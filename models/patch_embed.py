...
class Tokens2Image(nn.Module):
    def __init__(
        ...
        kernel_height, kernel_width = image_height // patch_height, image_width // patch_width
        self.conv_stem = nn.ConvTranspose2d(
            embedding_dim,
            channels,
            kernel_size=16,
            stride=16,
    ...
    def forward(self, x):
        B, N, D = x.shape
        assert N == self.patch_height * self.patch_width
        x = x.transpose(1, 2).view(B, D, self.patch_height, self.patch_width).contiguous()
        x = self.conv_stem(x)
        x = self.tanh(x)
        return x
